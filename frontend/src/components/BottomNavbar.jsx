    ...
    <nav className="fixed bottom-0 left-0 right-0 bg-white text-gray-500 border-t border-gray-200 shadow-lg">
      <div className="flex justify-around items-center h-16">
        {navItems.map((item) => (
          ...
          >
            <div className={`w-6 h-6 mb-1 group-hover:text-gray-700 ${activeTab === item.id ? 'text-blue-500' : 'text-teal-400'}`}>
              {item.icon}
            </div>
            <span className={`text-xs font-medium group-hover:text-gray-800 ${activeTab === item.id ? 'text-blue-500' : 'text-gray-600'}`}>
              {item.label}
            </span>
          </button>
        ))}
